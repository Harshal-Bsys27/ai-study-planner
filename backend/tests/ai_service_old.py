...
""",
    ...
    def generate_flashcards(self, topic: str, num_cards: int = 5) -> List[Dict]:
        ...
        # Use direct Gemini API (faster and more reliable)
        if GENAI_AVAILABLE:
            return self._generate_flashcards_genai(topic, num_cards)
        else:
            logger.error("❌ google-generativeai not installed")
            return []

    def _generate_flashcards_genai(self, topic: str, num_cards: int) -> List[Dict]:
        """Generate flashcards using direct Google Generative AI (most reliable)."""
        ...
        try:
            ...
            prompt = (
                ...
                "- Valid JSON array only, no markdown code blocks\n"
            ...
            if raw_text:
                cards = self._parse_json_list(raw_text)
                if cards and len(cards) > 0:
                    logger.info(f"✅ Generated {len(cards)} flashcards for '{topic}'")
                    return cards
                else:
                    logger.warning(f"⚠️ Could not parse JSON from response: {raw_text[:150]}")
                    raise Exception(f"Could not parse flashcard data from API response")
            else:
            ...
            if "quota" in error_msg.lower() or "resource_exhausted" in error_msg.lower():
                ...
                raise Exception(f"API quota exceeded - free tier limit reached. Please wait a while and try again.")
            else:
                logger.error(f"❌ Gemini flashcard generation failed: {error_msg[:200]}")
                raise Exception(f"Flashcard generation failed: {error_msg[:100]}")
